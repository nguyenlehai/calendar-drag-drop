...
const Calendar = () => {
  ...
  const handleAddExercise = (eventId: string) => {
    const newExercise: IExcercise = {
      id: `exercise-${Date.now()}`,
      title: 'New Exercise',
      quantity: 1,
      description: 'Add description',
      eventId,
      date: events.find(e => e.id === eventId)?.date || ''
    };

    setEvents(prev => prev.map(event => 
      event.id === eventId 
        ? { ...event, exercises: [...event.exercises, newExercise] }
        : event
    ));
  };
        ...
        <DndContext sensors={sensors} onDragEnd={handleDragEnd} onDragStart={handleDragStart} onDragOver={handleDragOver}>
          {DAY_OF_WEEK.map((d, idx) => (
            <Day 
              ...
              date={d.date} 
              id={d.id} 
              events={events.filter(e => e.date === d.id)}
          ...
          <DragOverlay dropAnimation={dropAnimation} adjustScale={false}>
            ...
            {activeItemData && activeItemType === ItemType.EVENT && (
              <DayEvent 
                data={activeItemData as IEvent} 
                onAddExercise={handleAddExercise}
              />
            )}
